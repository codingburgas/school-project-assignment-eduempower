...
void progLesson1() {
    ...
    Texture2D background = LoadTexture("../resources/bg-programming-if-else.png");
    ...
    Texture2D progButtonImg = LoadTexture("../resources/Mathematics.png");
    ...
    int xOffsetLessons = xOffsetMathematics + progButtonImg.width + buttonGap - 260; // Right of physics button
    ...
    while (!WindowShouldClose()) {
        ...
        DrawTexture(progButtonImg, xOffsetMathematics, yOffset, WHITE);
        ...
        if (CheckCollisionPointRec(GetMousePosition(), { (float)xOffsetMathematics, (float)yOffset, (float)progButtonImg.width, (float)progButtonImg.height }) ||
        ...
        if (IsMouseButtonPressed(MOUSE_LEFT_BUTTON) && CheckCollisionPointRec(GetMousePosition(), { (float)xOffsetMathematics, (float)yOffset, (float)progButtonImg.width, (float)progButtonImg.height })) {
            CloseWindow();
            progMenu();
        }
